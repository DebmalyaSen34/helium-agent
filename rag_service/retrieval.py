...
from rag_service.models import Chunk, RetrievedChunk
...
class HybridRetriever:
    """Hybrid-shaped retriever with deterministic lexical fallback.

    The first production slice keeps the interface ready for BGE dense/sparse
    retrieval and reranking, while remaining runnable without model downloads.
    """

    def __init__(self, chunks: list[Chunk]) -> None:
        self.chunks = chunks
        self.chunk_terms = [Counter(tokenize(chunk.text)) for chunk in chunks]
        self.filename_terms = [set(tokenize(chunk.file_name)) for chunk in chunks]
        self.location_terms = [set(tokenize(chunk.location.label)) for chunk in chunks]
    ...
    def search(self, query: str, max_results: int) -> list[RetrievedChunk]:
        ...
        for index, chunk in enumerate(self.chunks):
            ...
            if score > 0:
                scored.append(RetrievedChunk(chunk=chunk, score=score, reason="lexical_hybrid_fallback"))

        scored.sort(key=lambda item: (-item.score, item.chunk.file_name, item.chunk.index))
    ...
    def _score(self, query_terms: list[str], index: int) -> float:
        ...
        for term in query_terms:
            ...
            metadata_match = term in self.filename_terms[index] or term in self.location_terms[index]
            if freq == 0 and not metadata_match:
                continue
